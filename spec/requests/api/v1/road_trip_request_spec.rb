...
RSpec.describe 'RoadTrip API' do
  ...
  describe 'sad paths' do
    it 'impossible trip' do
      impossible_trip_params = {
        origin: "Denver, CO",
        destination: "London, UK",
        api_key: @user.api_key
      }
      post '/api/v1/road_trip', headers: @headers, params: impossible_trip_params.to_json
      expect(response).to be_successful
      trip = JSON.parse(response.body, symbolize_names: true)
      expect(trip).to be_a(Hash)
      expect(trip).to have_key(:data)
      ...
      expect(trip[:data][:attributes]).to have_key(:weather_at_eta)
      expect(trip[:data][:attributes][:weather_at_eta]).to be_a(Hash)
      expect(trip[:data][:attributes][:weather_at_eta]).to_not have_key(:temperature)
      expect(trip[:data][:attributes][:weather_at_eta]).to_not have_key(:conditions)
    end
